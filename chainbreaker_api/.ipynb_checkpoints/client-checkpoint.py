...
class ChainBreakerClient():
    ...
    def get_sexual_ads(self, language = "", website = "", start_date = "0001-01-01", end_date = "9999-01-01"): #, features = True, locations = False, comments = False, emails = False, names = False, phone = False, whatsapp = False):
        ...
        def get_df(info):
            ...
            columns = ["id_ad", "data_version", "author", "language", "link", "id_page", "title", "text", "category", "post_date", "extract_date", "website", "whatsapp", "email", "verified_ad", "prepayment", "promoted_ad", "external_website", "reviews_website", "phone", "country", "region", "city", "place", "latitude", "longitude", "zoom"]
            df = df[columns]
        ...
        res = requests.post(self._endpoint + "/api/data/get_ads?from_id=" + str(from_id), data = data, headers = headers).json()
        ...
        print("Progress: ", round(get_total_fetch(dataframes) / total_results * 100), 3)
